...
async def async_setup_entry(hass: HomeAssistant, entry: IsyConfigEntry) -> bool:
    ...
    # Maximum simultaneous HTTP requests to the controller. The fork's
    # default is strict serialization because ISY-994-era hardware is
    # effectively single-lane for reliable command traffic. 0 means "use
    # library auto-detection"; a positive integer overrides.
    max_concurrent_option = isy_options.get(
        CONF_MAX_CONCURRENT_COMMANDS, DEFAULT_MAX_CONCURRENT_COMMANDS
    )
    max_concurrent = max_concurrent_option if max_concurrent_option > 0 else None

    # Connect to ISY controller.
    isy = ISY(connection_info, max_concurrent=max_concurrent)
